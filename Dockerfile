...
RUN set -ex \
  ...
  && openssl req -x509 -newkey rsa:4096 -sha256 -days 3650 -nodes -keyout testpk.pem -out fullchain.pem \
       -subj "/C=GB/ST=London/L=London/O=Justin8/CN=my.example.com" \
...
LABEL \
    ...
    org.opencontainers.image.url="https://github.com/justin8/docker-http-https-echo" \
    org.opencontainers.image.documentation="https://github.com/justin8/docker-http-https-echo/blob/master/README.md" \
    org.opencontainers.image.source="https://github.com/justin8/docker-http-https-echo" \
    org.opencontainers.image.licenses="MIT"
WORKDIR /app
RUN apk upgrade --no-cache && rm -rf /usr/local/lib/node_modules
COPY --from=build /app /app
ENV HTTP_PORT=8080 HTTPS_PORT=8443 LOG_SERVER_PORT=8081
EXPOSE $HTTP_PORT $HTTPS_PORT $LOG_SERVER_PORT
USER 1000
CMD ["node", "./index.js"]
